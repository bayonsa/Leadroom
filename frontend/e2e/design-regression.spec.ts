...
test('workspace opens with content and advanced controls stay progressive', async ({ page }, testInfo) => {
  ...
  await expect(bothSource).toBeHidden()
  await expect(page.getByLabel('Results per search')).toBeHidden()
  await advanced.click()
  await expect(bothSource).toBeVisible()
  ...
  await expect(page.getByLabel('Results per search')).toHaveValue('100')
  await expect(page.getByLabel('Candidates per batch')).toHaveValue('500')
  ...
  await expect(page.getByText('Find up to 500 candidates for review')).toBeVisible()
