...
- **Adaptive fetching:** blocked or browser-challenged public pages can retry through Scrapling before the existing Playwright browser fallback.
...
The crawler remains on the candidate domain, prioritizes contact/about/service/location pages, and can traverse sitemap indexes. **Adaptive fetching** is enabled by default in a run's Advanced settings: Leadroom starts with its lightweight HTTP client, retries eligible failures through Scrapling, and only then opens the Playwright browser fallback. Redirect destinations, response types, and the 5 MB response limit are validated at every layer. It does not bypass authentication, paywalls, CAPTCHAs, or access controls.
...
FastAPI localhost API ---> SQLite runs/repository/compliance
        ...
        +-- bounded crawl + httpx/Scrapling/Playwright fetching
...
- `app/enrichment.py`, `app/scraper.py` - crawling, adaptive fetching, contacts, and ScrapeGraphAI
